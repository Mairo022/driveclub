...
import {capitalise} from "../../utils/stringFormatter";
...
export function Driver(): ReactElement {
    ...
    const {firstColumn, secondColumn} = columnsData(driver)

    function columnsData(driver: IDriverServer | undefined): IDriverColumns {
        return !driver
            ? {firstColumn: undefined, secondColumn: undefined}
            : {
                firstColumn: {money: driver.money, wins: driver.wins, podiums: driver.podiums, poles: driver.poles},
                secondColumn: {crashes: driver.crashes, infractions: driver.infr, "crashes/100km": driver.crashes_per_hundred_km, "infractions/100km": driver.infr_per_hundred_km}
            }
    }

    return (
        <article className="driver">
            <button className="back" style={{display: "none"}} onClick={() => navigate(-1)}>
            ...
            {isSuccess && firstColumn && secondColumn && <>
            <header className="heading">
                <h2 className="heading__driver">{driver.name}</h2>
                <p className="heading__driven">{driver.driven}km</p>
            </header>
            <div className="info">
